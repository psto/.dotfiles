...
local mru_opts = {
	ignore = function(path, ext)
		return
(string.find(path, "COMMIT_EDITMSG")) or (vim.tbl_contains(default_mru_ignore, ext))
	end,
}
...
local section = {
	...
	footer = {
		...
		val = {
			...
			{ type = "padding", val = 1 },
			file_button("~/.config/nvim/init.lua", "c"),
			file_button("~/.config/tmux/tmux.conf", "t"),
			file_button("~/.config/i3/config", "i"),
		},
	},
}
