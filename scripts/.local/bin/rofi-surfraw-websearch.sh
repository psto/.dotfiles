...
surfraw -browser=$BROWSER $(sr -elvi | awk -F'-' '{print $1}' | sed '/:/d' | awk '{$1=$1};1' | rofi -kb-row-select "Ctrl+q" -kb-row-tab "Control+space" -config ~/.config/rofi/searchmenu.rasi -dmenu -mesg ">>> Ctrl+p = Autocomplete" -i -p "rofi-surfraw-websearch: ")
